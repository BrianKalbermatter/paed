...
- [x] **`paed-lsp`: los errores del parser, subrayados en el editor — 2026-08-27.** `lang/lsp/main.c`, un servidor LSP que habla por stdin/stdout con cabecera `Content-Length`. NO tiene gramática propia: llama a `lang/src/parser.c`, el mismo que ejecuta — el error subrayado es EXACTAMENTE el que sale al correr. Implementa `initialize`, `didOpen`, `didChange`, `didClose`, `shutdown` y publica diagnósticos. Enganchado en `~/.config/helix/languages.toml`. Límites: `PAEDError` trae línea pero no columna (subraya la línea entera), y escribe el buffer a un temporal porque `paed_parse_file` solo lee de disco — que es lo correcto, revisa el texto SIN GUARDAR #lsp #parser
- [x] **`USAR <libreria>;` — módulos, 2026-08-27. NO es de cátedra.** Antes solo entraba UNA librería (`g_escena` se pisaba a sí misma) y la elegía el host por línea de comandos. Ahora el ARCHIVO pide lo que necesita, hasta 8 a la vez (`PAED_MAX_LIBS`). Va antes de `ACCION`, una por línea, repetir no es error, y el lenguaje siempre gana sobre la librería. Documentado en `docs/PAED.md` §9.bis marcado `[decidido]` #lenguaje #parser
- [x] **Tipo `EXPR` para parámetros de librería — 2026-08-27.** `NUM` valida con `strtod`, o sea que solo aceptaba números ESCRITOS: `x = 3` pasaba, `x = px + 1` no. Inservible para un juego, donde todo es calculado. `EXPR` se acepta tal cual y lo resuelve el intérprete en runtime. **`escena.json` sigue con `NUM` y tiene el problema sin arreglar** #lenguaje #parser
- [x] **Funciones del host: `paed_register_func` y `paed_eval` — 2026-08-27.** El host podía registrar PROCEDIMIENTOS pero no FUNCIONES, así que `SI TECLA('W') ENTONCES` no se podía escribir y no había forma de hacer un game loop en pseudocódigo. `expr.c` ya tenía el gancho genérico `expr_set_funcion`: solo hubo que caer a la tabla del host cuando el nombre no es del programa. `paed_eval` deja que el host resuelva el texto de un argumento sin exponer el `Entorno`, que sigue siendo privado del intérprete #lenguaje #evaluador
- [x] **Guarda: `sintaxis.json` cubre las palabras del parser — 2026-08-27.** Test en `tests/correr.sh` que saca con `rg` las palabras que compara `parser.c` y falla si alguna no está en el json. Nació porque agregué `USAR` al parser y no al json: los 49 tests pasaron, el lenguaje funcionó, y lo único roto era el color. **En su primera corrida encontró 5 huérfanas preexistentes**: `SUBACCION`, `FIN`, `FACCION`, `FINREGISTRO`, `FIN_SUBACCION` — todas grafías de cátedra que el resaltador nunca pintó. Es una heurística sobre el fuente: si el parser compara de otra forma, hay que agregar el patrón #tests #infra
- [x] **`_void/` borrado y sus lecciones rescatadas — 2026-08-27.** 20 archivos, 124 KB. Queda en el historial de git. Lo que valía la pena está en `docs/LECCIONES.md`: las tres listas de keywords desincronizadas que mataron la versión anterior, y por qué un resaltador no puede saber lo que sabe el parser. La gramática de tree-sitter salió antes a `paed/helix/`, que es donde `languages.toml` ya la buscaba #infra #docs
- [x] **La gramática de Helix distingue una llamada a función — 2026-08-27.** Regla `llamada` (un nombre PEGADO a un paréntesis, con `token.immediate`) y `declaracion_subaccion`. Los captures terminan en `.paed` para que el color no toque a ningún otro lenguaje: palabras reservadas en azul, lo que definís vos en celeste. **Ojo**: también pinta `CUBO` y `BILLBOARD` de una librería — la gramática no puede distinguirlas y no debe intentarlo, ver `docs/LECCIONES.md` #helix
- [x] **`docs/dicc_PAED_engine.md` — 2026-08-27.** Diccionario de la librería `mundo`: los once verbos con sus parámetros y valores por defecto, `USAR`, el esqueleto del game loop, los colores y una tabla de errores. El ejemplo que trae adentro se corrió antes de publicarlo. Escribirlo destapó que `CUBO` aceptaba 3 colores y `BILLBOARD` 7 #docs
...
- [x] **Frankly retirado**: `data/`, `docs/`, `tests/` y `ejercicios/` salen a la raíz; el intérprete bash y lo generado muerto van a `_void/` (borrado el 2026-08-27; lecciones en `docs/LECCIONES.md`). Documentado en `docs/ESTRUCTURA.md` #docs #infra
